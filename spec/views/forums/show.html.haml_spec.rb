...
describe "forums/show.html.haml" do
  ...
  before :each do
    ...
    @forum_that_should_be_shown = stub_model(Forum, :name => "First Forum", :subtitle => "My Subtitle", :category => @corresponding_category)#, :topics => @all_topics)
    ...
    # TODO: Not working properly, as in topic#show_spec
    assign( :topics, @all_topics)#@forum_that_should_be_shown.topics.paginate(:page => params[:page], :order => 'created_at DESC', :per_page => 10)  )
  ...
  it "should display a table with the first ten topics" do
    first_ten_topics = @all_topics[0..10]    
    first_ten_topics.should_not be_empty
    
    rendered.should have_selector('table tr') do |row|
      first_ten_topics.each do |topic|
        row.should have_selector('td a', :content => topic.title, :href => topic_path(topic))
      end
    end
